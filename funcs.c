...
//     Node *el = NULL;
//     createNode(&el);
//     el->info = (Info*)calloc(1, sizeof(Info));
//     char name[SIZE_OF_BUFF] = "";
//     char surname[SIZE_OF_BUFF] = "";
//     char dateOfBirth[SIZE_OF_BUFF] = "";
//     while (1)
//     {
//         printf("Enter name of student: ");
//         if (!scanf("%s", name))
//         {
//             printf("Invalid name of student! Please, try again!\n");
//             clearArr(name, SIZE_OF_BUFF);
//             continue;
//         }
//         else if (strncmp(name, "cancel", strlen("cancel")) == 0 || strncmp(name, "c", strlen("c")) == 0)
//         {
//             free(el->info);
//             free(el);
//             return 1;
...
//         else
//         {
//             el->info->mName = calloc(strlen(name), sizeof(char));
//             strncpy(el->info->mName, name, strlen(name));
//             break;
//         }
//     }
//     while (1)
...
//         if (!scanf("%s", surname))
//         {
//             printf("Invalid surname of student! Please, try again!\n");
//             clearArr(surname, SIZE_OF_BUFF);
//             continue;
//         }
//         else if (strncmp(surname, "cancel", strlen("cancel")) == 0 || strncmp(surname, "c", strlen("c")) == 0)
//         {
//             free(el->info->mName);
//             free(el->info);
//             free(el);
...
//         else
//         {
//             el->info->mSurname = calloc(strlen(surname), sizeof(char));
//             strncpy(el->info->mSurname, surname, strlen(surname));
//             break;
//         }
//     }
//     while (1)
//     {
//         printf("Enter date of birth of student (in format dd:mm:yyyy, dd - day, mm - month, yyyy - year): ");
//         if (!scanf("%s", ))
//     }
...
//                 "1. Open file with information about students;\n"
...
int fillStudent(Node *pNode, char *partsOfStr[], int arrSize)
{
    ...
    {
        pNode->info->mName = (char *)calloc(strlen(partsOfStr[INDEX_OF_NAME]), sizeof(char));
        strncpy(pNode->info->mName, partsOfStr[INDEX_OF_NAME], strlen(partsOfStr[INDEX_OF_NAME]));
    }
    else
        return -1;
    if (partsOfStr[INDEX_OF_SURNAME] != NULL)
    {
        pNode->info->mSurname = (char *)calloc(strlen(partsOfStr[INDEX_OF_SURNAME]), sizeof(char));
        strncpy(pNode->info->mSurname, partsOfStr[INDEX_OF_SURNAME], strlen(partsOfStr[INDEX_OF_SURNAME]));
    }
    else
        return -1;
    int day, month;
    if (partsOfStr[INDEX_OF_DATE_OF_BIRTH] != NULL && isCorrectDate(partsOfStr[INDEX_OF_DATE_OF_BIRTH]))
    {
        pNode->info->age = (char *)calloc(strlen(partsOfStr[INDEX_OF_DATE_OF_BIRTH]) + 1, sizeof(char));
        strncpy(pNode->info->age, partsOfStr[INDEX_OF_DATE_OF_BIRTH], strlen(partsOfStr[INDEX_OF_DATE_OF_BIRTH]));
    }
    else
        return -1;
...
void delStudents(Node ** pHead)
{
    ...
    {
        tmp2 = tmp1->pNext;
        if (tmp2->info->listOfMarks[INDEX_OF_SUBJ1] == MARK_3 && tmp2->info->listOfMarks[INDEX_OF_SUBJ3] == MARK_3)
        {
    ...
    if ((*pHead)->info->listOfMarks[INDEX_OF_SUBJ1] == MARK_3 && (*pHead)->info->listOfMarks[INDEX_OF_SUBJ3] == MARK_3 && ((*pHead)->pNext))
    {
        tmp2 = (*pHead)->pNext;
        freeInfo((*pHead));
        free((*pHead));
    ...
    if (tmp1->info->listOfMarks[INDEX_OF_SUBJ1] == MARK_3 && tmp1->info->listOfMarks[INDEX_OF_SUBJ3] == MARK_3)
    {
        freeInfo(tmp1);
        free(tmp1);
        tmp1 = NULL;
